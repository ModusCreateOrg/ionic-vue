...
const attrs: { [key: string]: any } = {
  ...
  'ion-router-view/name': {
    type: 'string | symbol',
    description: 'When a `<ion-router-view>` has a `name` prop, it will render the component with the corresponding name in the matched route record\'s components option.'
  },
  'ion-router-view/route': {
    description: 'When a `<ion-router-view>` has a `route` prop, it will use that resolved Route Location instead of the current location.'
  },
};

docs.components.filter(c => !ignoredTags.includes(c.tag)).forEach(c => {
  let tagName = c.tag;
  ...
  c.props.map(p => {
    if (navigableTags.includes(p.attr || '') && tagName !== 'ion-tab-button') {
    ...
    attrs[`${tagName}/${p.attr || paramCase(p.name)}`] = {
      type: p.type,
      description: p.docs.replace(/router-outlet|ion-nav/g, 'ion-router-view'),
